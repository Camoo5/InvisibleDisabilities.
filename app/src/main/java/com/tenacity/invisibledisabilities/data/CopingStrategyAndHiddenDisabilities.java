package com.tenacity.invisibledisabilities.data;

import androidx.room.Embedded;
import androidx.room.Relation;

import java.util.ArrayList;
import java.util.List;


public class CopingStrategyAndHiddenDisabilities {

    @Embedded
    private CopingStrategy copingStrategy;

    @Relation(parentColumn = "id", entityColumn = "copingstrategy_id")
    private List<HiddenDisability> hiddenDisabilities;

    public  CopingStrategyAndHiddenDisabilities(CopingStrategyAndHiddenDisabilities hiddenDisabilities) {
        this.hiddenDisabilities = new ArrayList<>();
    }



    public CopingStrategy getCopingStrategy() {
        return copingStrategy;
    }

    public void setCopingStrategy(CopingStrategy copingStrategy) {
        this.copingStrategy = copingStrategy;
    }

    public List<HiddenDisability> getHiddenDisabilities() {
        return hiddenDisabilities;
    }

    public void setHiddenDisabilities(List<HiddenDisability> hiddenDisabilities) {
        this.hiddenDisabilities = hiddenDisabilities;
    }
}